...
multi_func<-function(group, nCV, mets){
        ...
        # CorL[[p]] = CorR
        path0 = paste0("./outputs/sc1_part2/", group, "_", i, "_cv",p)  ##########<---------------------------change here
        dir.create(file.path(path0), recursive = T)
        saveRDS(CorR, file=paste0(path0, "/scenario1_all", ".rds"))  ##########<---------------------------change here
...
mets = mets0
multi_func(group="Control", nCV=25, mets = mets)  ##########<---------------------------change here
