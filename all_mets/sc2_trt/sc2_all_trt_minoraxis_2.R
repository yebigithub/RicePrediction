...
Gcs_trt = scale(geno_trt, scale=T, center = T)
Gchnt_trt = tcrossprod(Gcs_trt)/ncol(Gcs_trt)
EVD_Gchnt_trt <- eigen(Gchnt_trt)
...
multi_func<-function(group, nCV, mets){
        ...
        # CorL[[p]] = CorR
        path0 = paste0("./outputs/sc2_part2/", group, "_", i, "_cv",p)
        dir.create(file.path(path0), recursive = T)
        saveRDS(CorR, file=paste0(path0, "/scenario2_all", ".rds"))
...
mets = mets0
multi_func(group="Stress", nCV=25, mets = mets)