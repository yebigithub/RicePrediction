...
library(lme4)
library(readxl)
Grain_Size <- read_excel("~/Library/CloudStorage/OneDrive-VirginiaTech/Research/Codes/research/RiceUNLMetabolites/Trait/original/Grain_Size_HNT_JS.xlsx", 
    sheet = "AllSeeds")
...
for (i in 1:3){
formu = as.formula(paste0(traits[i],"~ 1 + (1|GSOR.ID) + (1|PlantNo.)"))
fit.lme = lmer(formula = formu, data = Grain_size.con)
summary(fit.lme)
temp = as.data.frame(VarCorr(fit.lme))

H2 = temp[1,4]/(temp[1,4]+temp[3,4]/rep.con+temp[2,4])
print(round(H2, 2))
}
